...
describe("cavalry charge abilities", () => {
  it("Lance Charge requires three hexes of movement, grants its declared ATK, and leaves the rider Exposed", () => {
    const { b, ctrl } = newBattle();
    ...
    rider.movedThisActivation = 3; rider.chargeMoved = 3;
    ...
    // the number lives in the ability data; what this asserts is that the effect applies it
    const declared = reg.ability("ABL_LANCE_CHARGE").effect["atk"] as number;
    expect(after).toBe(before + declared);
  ...
  it("Fleet Strike triggers off two hexes moved and does not force Exposed", () => {
    ...
    outrider.ap = 2;
    outrider.movedThisActivation = 2; outrider.chargeMoved = 2;
