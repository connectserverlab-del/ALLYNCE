...
/**
 * The expansion roster is the one shipped in `data/units/expansion.json`. It cannot be spotted by
 * the presence of `stars` any more: the card system gives every unit in the core roster a star
 * rating too, so the file itself is what says which units these rules are about.
 */
const EXPANSION_IDS = new Set<string>(
  JSON.parse(readFileSync(resolve(ROOT, "data/units/expansion.json"), "utf8")).map((u: { id: string }) => u.id),
);
const expansion = all.filter((u) => EXPANSION_IDS.has(u.id));
const tenStar = expansion.filter((u) => u.stars === 10);
...
describe("expansion roster", () => {
  ...
  it("requested per-faction expansion counts are met", () => {
    ...
    const ten = (f: string) => expansion.filter((u) => u.faction === f && u.stars === 10).length;
